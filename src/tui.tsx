...
function App({ rows, layout, options, actions }: Props) {
  ...
  return <Box flexDirection="column" paddingX={1}>
    ...
    <Box marginTop={1} flexDirection="column">
      ...
      {filtered.map((item, index) => <Text key={`${item.version}-${index}`} inverse={index === selected && !options.noColor} color={!options.noColor && process.env.NO_COLOR === undefined && item.status?.includes('latest') ? 'green' : undefined}>{index === selected ? '› ' : '  '}{item.version}{layout.statuses ? `  ${item.status ?? ''}` : ''}{options.verbose ? `  ${item.detail ?? ''}` : ''}</Text>)}
    </Box>
